	...
	const fetchKerns = async () => {
		try {
			...
			const response = await axios.get(
				'http://72.56.233.251:4200/kern/get-kern-all',
				{
					headers: {
				...
				<div className='flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4'>
					<div>
						...
						<p className='text-gray-500 max-w-2xl leading-relaxed'>
							База образцов керна, их минералогии, текстурных параметров,
							пористости и проницаемости.
						</p>
					</div>

					<Link
						href={'/kern/create-kern'}
							...
							<tbody className='text-sm text-gray-700'>
								{loading ? (
									...
									tableData.map(row => (
										...
										>
											<td className='p-4 font-medium border-r border-gray-100 text-[#003366]'>
												{row.coreId}
											</td>
											...
											<td className='p-4 border-r border-gray-100 font-mono'>
												{row.depth}
											</td>
											<td className='p-4 border-r border-gray-100'>
												{row.formation}
